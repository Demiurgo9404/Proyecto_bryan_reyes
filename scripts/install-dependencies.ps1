...
function Install-NodeJS {
    if (-not (Test-CommandExists "node")) {
        ...
        # Descargar e instalar Node.js usando winget si está disponible
        if (Test-CommandExists "winget") {
            winget install OpenJS.NodeJS.LTS
        } 
        # Si no hay winget, intentar con el instalador web
        else {
...
function Install-FrontendDependencies {
    ...
    try {
        ...
        # Verificar si hay un package.json
        ...
        # Si hay un script de postinstall, ejecutarlo
...
function Install-BackendDependencies {
    ...
    try {
        ...
        # Verificar si es un proyecto Node.js o .NET
        if (Test-Path "package.json") {
            # Proyecto Node.js
            Write-Status "Instalando dependencias de Node.js..."
            npm install --no-fund --no-audit --prefer-offline
            
            # Instalar dependencias de desarrollo si hay un package-lock.json
        ...
        if ($csprojFiles) {
            ...
            # Instalar herramientas globales si es necesario
            $tools = @("dotnet-ef")
...
# Ejecutar instalación con parámetros por defecto
try {
